...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (!empty($_POST['method']) || !empty($_POST['paypal_transaction_id']))) {
   ...
   if($check_cart->rowCount() > 0){
      if($address == ''){
         ...
         $insert_order = $conn->prepare("INSERT INTO `orders` (user_id, name, number, email, method, address, total_products, total_price, paypal_transaction_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
         $insert_order->execute([$user_id, $name, $number, $email, $method, $address, $total_products, $total_price, $paypal_transaction_id]);
...
<head>
   ...
   <script src="https://www.paypal.com/sdk/js?client-id=AaLergoLwfqKDH_0V_TZzZAO2DJee9g4G4Gtm5yLfA4DvYZXIoNTYUddyqNC2Vkfj9GBXqpauRZVC63U&currency=USD&components=buttons,funding-eligibility"></script>
</head>
<body>
...
<form id="mainForm" action="" method="post">
   ...
   <input type="hidden" name="method" id="paypal_method" value="">
   <div class="user-info">
      <h3>your info</h3>
      ...
      <input type="submit" value="place order" class="btn <?php if($fetch_profile['address'] == ''){echo 'disabled';} ?>" style="width:100%; background:var(--red); color:var(--white);" id="submitBtn">
   </div>
</form>
</section>
...
paypal.Buttons({
   createOrder: function(data, actions) {
      return actions.order.create({
         purchase_units: [{ amount: { value: "<?= $grand_total ?>" } }]
      });
   },
   onApprove: function(data, actions) {
      return actions.order.capture().then(function(details) {
         document.getElementById("paypal_transaction_id").value = details.id;
         document.getElementById("paypal_method").value = "paypal";
